...
def train(net_factory,prefix,end_epoch,base_dir,display,base_lr):
    ...
    if os.path.exists(logs_dir) == False:
        os.mkdir(logs_dir)
    #模型训练
    # tf.add_to_collection("new_way", [train_op,lr_op])
    init = tf.global_variables_initializer()
    sess = tf.Session()
    ckpt1=tf.train.latest_checkpoint('../model/RNet')
    print(ckpt1)
    saver = tf.train.import_meta_graph( '../model/RNet/RNet-22.meta')
    print(saver)
    saver.restore(sess, ckpt1)
    sess.run(init)
    graph = tf.get_default_graph()
    # y = tf.get_collection("new_way")
    ...
    i = 0
    #总数据除以批大小等于一共几批数据再乘以迭代次数等于总的迭代次数
...
def optimize(base_lr,loss,data_num):
    ...
    #梯度下降算法，创建一个优化类Optimizer
    optimizer = tf.train.MomentumOptimizer(lr_op, 0.9)
    #调用优化类的tf.train.Optimizer.minimize()来更新节点
