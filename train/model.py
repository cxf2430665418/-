...
def P_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    ...
    with tf.variable_scope('PNet'):
        ...
        with slim.arg_scope([slim.conv2d],activation_fn=prelu,
                           ...
                           padding='VALID'):
            # 卷积层（inputs，卷积核个数10，卷积核维度3，名称conv2)
            conv1=slim.conv2d(inputs,10,3,scope='conv1')
            # 池化层（kernel_size用于指定卷积核的维度（卷积核的宽度，卷积核的高度），
            # stride为卷积时在图像每一维的步长，padding为padding的方式选择，VALID或者SAME）
            net=slim.max_pool2d(conv1,kernel_size=[2,2],stride=2,padding='SAME',scope='pool1')
            # print(net.get_shape())
            ...
            if training:
                ...
                return cls_pro_test,bbox_pred_test,landmark_pred_test,conv1
...
def R_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    ...
    with tf.variable_scope('RNet'):
        with slim.arg_scope([slim.conv2d],
                           ...
                           weights_regularizer=slim.l2_regularizer(0.0005),
                           padding='VALID'):
            conv1=slim.conv2d(inputs,28,3,scope='conv1')
            net=slim.max_pool2d(conv1,kernel_size=[3,3],stride=2,padding='SAME',scope='pool1')
            ...
            if training:
                ...
                return cls_prob,bbox_pred,landmark_pred,conv1
...
def O_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    ...
    with tf.variable_scope('ONet'):
        with slim.arg_scope([slim.conv2d],
                           ...
                           weights_regularizer=slim.l2_regularizer(0.0005),
                           padding='VALID'):
            conv1=slim.conv2d(inputs,32,3,scope='conv1')
            net=slim.max_pool2d(conv1,kernel_size=[3,3],stride=2,padding='SAME',scope='pool1')
...
def cls_ohem(cls_prob,label):
    ...
    返回值：
      损失
    '''
    # 创建一个类型为labal但所有元素为0的张量
    zeros=tf.zeros_like(label)
    #只把pos的label设定为1,其余都为0(-1，-2<0),tf.where()判断每个元素是否<0，是的话就对应位置变为0，否则变为label对应位置
    label_filter_invalid=tf.where(tf.less(label,0),zeros,label)
    #类别size[2*batch]，返回cls_prob中元素的个数
    ...
    #真实标签对应的概率，tf.gather：用一个一维的索引数组，将张量中对应索引的向量提取出来
    label_prob=tf.squeeze(tf.gather(cls_prob_reshpae,indices_))
    #当真实值为1时，交叉熵损失等于-log(y)
    ...
    #找到loss的最后一维中最大的K个数，返回这些最大的数和它们的下标
