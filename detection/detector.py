...
class Detector:
    ...
    def __init__(self,net_factory,data_size,batch_size,model_path):
        ...
        with graph.as_default():
            ...
            self.cls_prob, self.bbox_pred, self.landmark_pred,self.conv1= net_factory(self.image_op, training=False)
    ...
    def predict(self,databatch):
        ...
        for idx,data in enumerate(minibatch):
            ...
            if m<batch_size:
                ...
                data=data[keep_inds]
                real_size=m
            cls_prob,bbox_pred,landmark_pred,conv1=self.sess.run([self.cls_prob, self.bbox_pred,self.landmark_pred,self.conv1],
                                                          feed_dict={self.image_op: data})
            print(conv1.shape)
            '''
            for i in range(3):
                plt.matshow(conv1[0, :, :, i])
                plt.title(str(i) + "rnet conv1")
                plt.colorbar()
                plt.show()'''
